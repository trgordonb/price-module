...
```
Current, After the price message is received, the LastTradedPx is used as token price for keeping, furthermore, in order to reduce the record keep in database, if LastTradedPx doesn't change in compare to the latest updated price or LastTradedPx <=0,the message will be discard.
